...
function stripForbiddenAddress(value = '') {
  return String(value || '')
    ...
    .replace(/10263\s+Gandy\s+Blvd\s+N\s+#116/gi, '')
    .replace(/St\.\s*Petersburg,?\s*FL,?\s*,?\s*US/gi, 'St. Petersburg, FL, US')
    .replace(/St\.\s*Petersburg,?\s*FL,?\s*/gi, 'St. Petersburg, FL')
    .replace(/\b\b/g, '')
    .replace(/[ \t]{2,}/g, ' ')
    .trim();
}
...
export function renderCampaignHtml({ campaign, profile, contact = {}, unsubscribeUrl = '' }) {
  ...
  const borderRadius = removeEmDashes(cleanString(layout.border_radius || '26px', 16)) || '26px';
...
export function renderPlainText({ campaign, profile, contact = {}, unsubscribeUrl = '' }) {
  ...
  const paragraphs = Array.isArray(campaign.body_paragraphs) ? campaign.body_paragraphs : [campaign.plain_text || ''];
  paragraphs.forEach((paragraph) => lines.push(personalize(removeEmDashes(cleanString(paragraph, 2000)), contact)));
